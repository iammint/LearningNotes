...
All `--mode development/production` => needs to change to `--env.mode development/production` in the `package.json` file. And in the `webpack.config.js` file, we need to add the following:
...
//Or in the ES6 destructuring syntax:
module.exports = ({ mode }) => {
  if(mode == "development") {
    return {}
  } else {
