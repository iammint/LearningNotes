...
### How can you actually access environment variables from the command line?

1. In the `packages.json` file, `--mode` => `--env.mode`
> You can use the `--env` flag in the `npm run build` command.